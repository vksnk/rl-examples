...
class PyTorchToTF(gym.ObservationWrapper):
    def __init__(self, env):
        super(PyTorchToTF, self).__init__(env)
        old_shape = self.observation_space.shape
        self.observation_space = gym.spaces.Box(low=0.0, high=1.0, shape=(old_shape[1], old_shape[2], old_shape[0]),
                                                dtype=np.float32)
        # self.observation_space = gym.spaces.Box(low=0, high=255, shape=(old_shape[1], old_shape[2], old_shape[0]),
        #                                         dtype=np.uint8)
...
class BufferWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        # print(type(self.buffer), self.buffer.dtype, self.buffer.shape)
        return self.buffer
